# docs：技术文档索引

本目录是公共技术设计和项目口径的唯一汇总区。

## 核心文档

- `00_project_charter.md`：赛题定位、范围、阶段和工程原则。
- `01_git_project_setup_summary.md`：仓库最初搭建记录。
- `02_system_architecture.md`：系统分层、数据流、状态机和适配边界。
- `03_module_interfaces.md`：核心数据契约与外部集成接口。
- `04_quickstart.md`：运行、测试和结果解释。
- `05_open_source_research.md`：GitHub 候选方案、许可证和引入决策。
- `06_p1_coverage_and_animation.md`：全覆盖算法、逐帧数据和离线动画设计。
- `07_implementation_roadmap.md`：后续里程碑、交付物和验收门槛。
- `08_ros2_environment_and_bridge.md`：Humble/Fortress 环境决策、Docker/Pixi 复现路径、ROS2 回放桥和验证边界。
- `09_rdk_tros_deployment.md`：RDK X5/X3 的 OS 3.x/TROS-Humble 部署、PC—板端分工和实机验收标准。
- `README0821.md`：2026-08-21 当日完整成果、路径说明、启动方法、验证结果与下一步。

## 维护要求

- 技术选择、模块边界、接口字段或指标定义变化时，同一次改动更新对应文档。
- 已实现、已验证、设计中和待硬件验证必须明确标注。
- 外部资料应记录来源和访问日期；不要大段复制受版权保护的内容。
- 个人临时笔记整理后再进入本目录，禁止保存密钥或私有服务器信息。
