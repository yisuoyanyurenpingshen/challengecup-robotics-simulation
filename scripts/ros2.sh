...
case "${1:-shell}" in
  ...
  *)
    cat >&2 <<'USAGE'
用法：bash scripts/ros2.sh {install|shell|build|test|demo|verify|gazebo|gazebo-verify|trash-verify|camera-verify|perception-verify|position-verify|lidar-verify|drive|drive-verify|drive-gui|doctor|run}
